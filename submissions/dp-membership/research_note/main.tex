...
Moderate DP & 2.0 & 9.4 & $0.805 \pm 0.091$ & $0.543 \pm 0.011$ \\
Strong DP & 5.0 & 3.4 & $0.709 \pm 0.118$ & $0.521 \pm 0.005$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{DP-SGD effectively mitigates the attack.} Even weak DP ($\sigma{=}0.5$) dramatically reduces attack AUC from 0.664 to 0.532. Strong DP ($\sigma{=}5.0$) further reduces it to 0.521, near random guessing.
...
All experiments are reproducible via the accompanying SKILL.md. The DP-SGD implementation uses no external DP libraries. Seeds are fixed at [42, 123, 456]. Dependencies are pinned: PyTorch 2.6.0, NumPy 2.2.4. Runtime is approximately 22 seconds on a modern CPU.
