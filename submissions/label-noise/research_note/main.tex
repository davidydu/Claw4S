...
All experiments are fully reproducible from a single \texttt{run.py} script in about 84 seconds on the verified CPU run in this repository.
\end{abstract}
...
\begin{itemize}
    \item \textbf{Shallow-wide:} 1 hidden layer, width 200 (3,205 params)
    \item \textbf{Medium:} 2 hidden layers, width 70 (6,095 params)
    \item \textbf{Deep-narrow:} 4 hidden layers, width 35 (4,345 params)
\end{itemize}
...
All experiments run from a single command (\texttt{.venv/bin/python run.py}) in about 84 seconds on the verified Python 3.13.5 / CPU execution in this repository.
