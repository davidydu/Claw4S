...
(3)~there is a modest negative Pearson correlation between Zipf exponent and compression ratio ($r \approx -0.34$, $p \approx 0.044$), while the rank-based Spearman test is not significant ($\rho \approx -0.26$, $p \approx 0.124$), suggesting that more Zipfian distributions may be associated with \emph{lower} compression efficiency.
...
The correlation between $\alpha$ and compression ratio is modestly negative in the Pearson analysis, but less stable in the rank-based Spearman analysis:
\begin{itemize}
  \item Pearson $r = -0.337$ ($p = 0.044$)
  \item Spearman $\rho = -0.261$ ($p = 0.124$)
\end{itemize}

This is a counterintuitive pattern, but it should be interpreted cautiously because only the Pearson test reaches conventional significance.
