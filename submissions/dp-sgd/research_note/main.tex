...
\title{The Privacy Cliff Is a Clipping Cliff: \\
Disentangling Noise and Clipping in DP-SGD}
...
training runs, we find that the commonly described ``privacy cliff'' ---
where utility collapses below a critical $\varepsilon$ --- is better
understood as a \textbf{clipping cliff}. With well-tuned clipping ($C=1.0$),
accuracy remains within 5\% of the non-private baseline ($99.3\%$) even at
$\varepsilon < 1$. With aggressive clipping ($C=0.1$), accuracy drops to
$\sim$81\% regardless of noise level, while with loose clipping ($C=10.0$),
the noise-driven degradation becomes pronounced at high $\sigma$. These
results demonstrate that gradient clipping --- not privacy noise --- is the
primary bottleneck in DP-SGD and that the privacy-utility tradeoff is
largely a clipping-utility tradeoff.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Runtime:} 44.6 seconds on CPU (Apple Silicon).
\end{itemize}
...
\begin{figure}[h]
    ...
    \caption{Privacy-utility tradeoff across clipping norms. The ``cliff''
    at low $\varepsilon$ is dominated by the choice of $C$, not $\sigma$.}
    \label{fig:privacy_utility}
\end{figure}

\begin{itemize}
    \item \textbf{$C = 0.1$ (aggressive clipping):} Accuracy is flat at
    $\sim$81\% across all $\varepsilon$ values. Clipping is so severe that
    ...
    $94$--$99\%$ across the full $\varepsilon$ range, including
    $\varepsilon < 1$. At $\varepsilon = 0.87$ ($\sigma = 10$), accuracy
    is still $94.3\%$ --- only 5 points below the baseline.

    \item \textbf{$C = 10.0$ (loose clipping):} Shows the classic
    noise-driven cliff. Accuracy is $99\%$ at $\varepsilon = 22{,}446$ but
    drops to $63\%$ at $\varepsilon = 0.87$. Because clipping preserves
...
\midrule
0.01  & 22{,}446 & $81.3 \pm 7.8$ & $99.3 \pm 0.5$ & $99.3 \pm 0.5$ \\
0.10  & 23.0     & $81.3 \pm 7.8$ & $99.3 \pm 0.5$ & $99.0 \pm 0.8$ \\
1.00  & 4.80     & $82.0 \pm 8.0$ & $98.7 \pm 1.2$ & $97.7 \pm 1.9$ \\
2.00  & 1.33     & $81.0 \pm 6.5$ & $98.3 \pm 1.7$ & $92.3 \pm 1.7$ \\
5.00  & 1.06     & $80.7 \pm 7.6$ & $98.3 \pm 2.4$ & $80.0 \pm 2.2$ \\
10.0  & 0.87     & $80.3 \pm 7.4$ & $94.3 \pm 1.2$ & $63.0 \pm 6.4$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{The privacy cliff is a clipping cliff.} Our results show that the
dramatic accuracy collapse commonly attributed to ``strong privacy'' is
largely an artifact of poor clipping norm selection. With appropriate $C$,
DP-SGD achieves near-baseline accuracy even at $\varepsilon < 1$ on our
synthetic task.
...
The complete experiment (63 DP runs + 3 baselines) runs in under 1 minute
