...
We find that \textbf{moderate depth is the most robust default}: depth-2 belongs to the best-performing set in all six task-budget conditions, converges 4--11$\times$ faster than single-layer networks on sparse parity, and delivers the best generalization on smooth regression.
Very deep networks (8 layers) suffer from optimization instability at small budgets (75.9\% vs.\ 100\% accuracy on parity at 5K params), and single-layer networks converge slowly on structured tasks despite eventually reaching high accuracy.
...
\begin{table}[h]
\centering
\caption{Convergence speed on sparse parity: epochs to reach 85\% test accuracy. Depth 2--4 converge 4--11$\times$ faster than depth 1.}
...
The key finding is in convergence speed (Table~\ref{tab:parity-speed}). While most configurations eventually reach near-perfect accuracy (Table~\ref{tab:parity-acc}), depth-2 and depth-4 networks converge 4--11$\times$ faster than depth-1 networks at the same parameter budget (e.g., at 50K params, depth-4 reaches 85\% at epoch 36 vs.\ 411 for depth-1). This aligns with theory: deeper networks can compute parity with fewer parameters per layer. However, depth-8 networks at the smallest budget (5K params, width=25) achieve only 75.9\% test accuracy, demonstrating the optimization difficulty of very deep narrow networks without skip connections.
...
\paragraph{Moderate depth is a robust default.} Across both tasks and all budgets, depth-2 networks achieve the best or near-best performance, and they appear in the best-performing set for every task-budget pair. This suggests that for practical MLP deployment under parameter constraints, two hidden layers is a robust default choice.
...
\paragraph{Training stability degrades with depth.} At the 5K budget, depth-8 networks (width=25--26) achieve only 75.9\% on parity and never reach 0.90 $R^2$ on regression. With only $\sim$25 neurons per layer, optimization becomes fragile, especially without skip connections or normalization.
