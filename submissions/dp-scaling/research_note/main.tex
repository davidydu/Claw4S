...
We demonstrate that neural scaling laws persist under DP-SGD with high goodness of fit ($R^2 > 0.95$). On our synthetic classification task, DP-SGD raises absolute loss levels while also yielding a slightly higher fitted $\alpha$ than the non-private baseline. Because all runs already achieve 100\% test accuracy, we interpret this as a result about loss scaling and calibration on an easy task, not a general claim that privacy improves learning efficiency. Moderate ($\sigma = 1.0$) and strong ($\sigma = 3.0$) DP show nearly identical exponents, consistent with a clipping-dominated regime on this setup. Future work should validate these patterns on harder tasks (e.g., natural images, language), at larger model scales, and with formal $(\varepsilon, \delta)$ privacy accounting.
