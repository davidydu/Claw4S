...
\title{Adversarial Robustness Scaling in Small Neural Networks:\\
Capacity-Independent Vulnerability on Synthetic Tasks}
...
we find that the robustness gap --- the difference between clean and adversarial accuracy ---
is largely \emph{independent} of model capacity.
On the circles task, the correlation between log parameter count and mean robustness gap
is near zero ($r = -0.06$ for FGSM), while on the moons task a moderate negative
correlation ($r = -0.67$) suggests that larger models may be slightly \emph{more} robust.
These results indicate that in the small-model regime, adversarial vulnerability is governed
primarily by task geometry rather than model capacity, contrasting with large-scale findings
where increased capacity improves robustness.
\end{abstract}
...
\subsection{Robustness Gap Is Capacity-Independent}

Table~\ref{tab:circles} shows the per-width results on the circles task.
The robustness gap is remarkably stable across a 750$\times$ range of parameter counts:
the mean FGSM gap ranges from 0.302 to 0.328, and the mean PGD gap from 0.313 to 0.335.
The correlation between $\log_{10}$(params) and mean robustness gap is
$r = -0.06$ (FGSM) and $r = -0.16$ (PGD) --- effectively zero.
...
512   & 265{,}218& $0.940 \pm 0.014$ & $0.326 \pm 0.009$ & $0.334 \pm 0.010$ \\
\bottomrule
\end{tabular}
\end{table}

On the moons task, FGSM gap shows a moderate negative correlation ($r = -0.67$),
suggesting larger models may be slightly \emph{more} robust. PGD gap correlation
is weaker ($r = -0.18$).
...
\subsection{Why Capacity Doesn't Matter Here}

The key finding --- that robustness gap is independent of model capacity in the small-model
regime --- can be understood through the lens of task complexity.
...
Once the boundary is learned, the adversarial vulnerability is determined by the
\emph{geometry} of that boundary relative to the input perturbation budget,
not by the number of parameters used to represent it.
...
to essentially the same function, differing only in smoothness of the boundary
...
\begin{enumerate}
    ...
    \item \textbf{Sufficient capacity} ($h_{\min} \leq h \leq h_{\text{large}}$):
          All models learn similar decision boundaries; robustness gap is capacity-independent.
...
The pipeline runs in under 90 seconds on a single CPU core.
...
is largely independent of model capacity.
Across two synthetic tasks, six model sizes spanning 750$\times$ parameter counts,
and three random seeds, the robustness gap under FGSM and PGD attacks remains stable
or slightly decreases with model size.
This contrasts with the common assumption that larger models are inherently more
vulnerable and with large-scale findings showing improved robustness with scale,
suggesting a nuanced, regime-dependent relationship between model capacity and
