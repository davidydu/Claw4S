...
For each fit, we generate $B = 1000$ synthetic datasets by adding Gaussian noise (with standard deviation estimated from residuals) to the fitted curve, refit, and extract 95\% confidence intervals from the bootstrap distribution.
...
This confirms that improvements in loss do not proportionally translate to improvements in task accuracy, and vice versa.
...
\begin{itemize}
  \item \textbf{Add a model family:} Add a new dictionary to \texttt{src/data.py} following the existing format. The analysis auto-detects all families.
  ...
  \item \textbf{Change bootstrap samples:} Adjust \texttt{n\_bootstrap} in \texttt{run.py} (default: 1000).
\end{itemize}
