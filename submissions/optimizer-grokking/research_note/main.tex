...
\begin{abstract}
Grokking---the phenomenon where neural networks generalize long after memorizing training data---has been primarily studied under weight decay variation with a single optimizer. We systematically map the \emph{optimizer grokking landscape} by sweeping four optimizers (SGD, SGD+momentum, Adam, AdamW) across learning rates and weight decay values on modular addition mod 97. Across 36 configurations (4 optimizers $\times$ 3 learning rates $\times$ 3 weight decays, 750 epochs each), we find that AdamW produces the most reliable grokking (5/9 configs), Adam groks only without explicit weight decay (1/9), SGD+momentum memorizes but never groks, and vanilla SGD fails entirely. A striking asymmetry emerges: Adam with weight decay \emph{collapses} while AdamW with decoupled weight decay \emph{groks}---highlighting that the mechanism of regularization, not just its presence, determines generalization. Our fully reproducible experiment runs in under 10 minutes on CPU.
\end{abstract}
...
Each optimizer is paired with 3 learning rates $\in \{0.1, 0.03, 0.01\}$ and 3 weight decay values $\in \{0, 0.01, 0.1\}$, yielding $4 \times 3 \times 3 = 36$ total configurations. Each run trains for 750 epochs with batch size 512 and mini-batch stochastic updates.

\subsection{Grokking Detection}

We classify each run's outcome:
\begin{itemize}
...
The \emph{grokking delay} is defined as the epoch gap between memorization and generalization.
...
\begin{itemize}
    \item \textbf{AdamW} is the most reliable grokker (5/9 configs grok), succeeding at both tested learning rates ($0.03$ and $0.01$) when weight decay is present.
...
\begin{figure}[h]
    ...
    \caption{Grokking landscape: outcome (green=grokking, red=memorization, gray=failure) across optimizer $\times$ (learning rate, weight decay) configurations. Numbers show final test accuracy.}
    \label{fig:heatmap}
\end{figure}

\subsection{Training Dynamics}

Figure~\ref{fig:curves} shows representative training curves for each outcome type. The grokking example exhibits the characteristic pattern: train accuracy reaches near-100\% within the first few hundred epochs, while test accuracy remains at chance level before suddenly rising to high accuracy.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{../results/training_curves.png}
    \caption{Representative training curves showing grokking (left), memorization (center), and failure (right) dynamics.}
    \label{fig:curves}
\end{figure}

\subsection{The Adam vs.\ AdamW Paradox}

The most striking finding is the opposite effect of weight decay on Adam and AdamW. For Adam, adding weight decay (implemented as L2 regularization) causes training to collapse entirely---the model cannot even memorize. For AdamW, adding decoupled weight decay \emph{enables} grokking and accelerates generalization.
...
This study is limited to a single task (addition mod 97), a single architecture (2-layer MLP), and a fixed train/test split. The grokking landscape may differ for other modular operations, larger models, or different data fractions. Our 750-epoch budget may miss very late grokking events, though we verified that key grokking transitions occur within the first 600 epochs. The learning rate grid ($0.1$, $0.03$, $0.01$) may not be optimal for SGD variants, which might benefit from very different scales.

\paragraph{Reproducibility.}
All code, data generation, and analysis are fully deterministic (seed=42) and run in under 10 minutes on a single CPU. The accompanying SKILL.md provides step-by-step instructions for an AI agent to reproduce all results.

\section{Conclusion}

We present a systematic mapping of the optimizer grokking landscape on modular arithmetic. AdamW emerges as the most reliable optimizer for inducing grokking (5/9 configs), while vanilla SGD fails entirely and SGD+momentum only memorizes. The most surprising finding is the Adam/AdamW paradox: weight decay helps AdamW grok but causes Adam to collapse, highlighting that decoupled weight decay and L2 regularization are fundamentally different mechanisms in the grokking regime. These findings establish optimizer selection as a first-order consideration in studying and inducing grokking.
