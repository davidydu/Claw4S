...
\begin{enumerate}
  ...
  \item An \emph{agent-executable skill} (\texttt{SKILL.md}) that encodes the full experimental pipeline so that any AI coding agent can reproduce all 324 simulations and analyses from scratch.
\end{enumerate}
...
Table~\ref{tab:heatmap} shows the mean collusion index $\Delta$ for the e-commerce preset.
Values in bold exceed $\Delta > 0.1$; negative values indicate prices below Nash equilibrium.

\begin{table}[h]
\centering
\small
\caption{Mean collusion index $\Delta$ (e-commerce preset, no shock, mean over 3 seeds). Bold: $\Delta > 0.1$.}
\label{tab:heatmap}
\begin{tabular}{lccc}
\toprule
\textbf{Matchup} & \textbf{$M=1$} & \textbf{$M=3$} & \textbf{$M=5$} \\
\midrule
Q vs.\ Q         & 0.071 & \textbf{0.109} & \textbf{0.108} \\
SARSA vs.\ SARSA & 0.012 & \textbf{0.111} & \textbf{0.103} \\
PG vs.\ PG       & $-0.437$ & $-0.496$ & $-0.497$ \\
Q vs.\ SARSA     & 0.081 & \textbf{0.105} & 0.091 \\
...
\centering
\small
\caption{Pairwise auditor agreement (no-shock runs, $n = 162$) and panel F1 vs.\ $\Delta > 0.1$ ground truth.}
\label{tab:agreement}
\begin{tabular}{lcc}
\toprule
...
Q-learner $\times$ Q-learner results replicate Calvano et al.~\cite{calvano2020artificial} across all three market presets, confirming that tacit collusion is a robust property of independent Q-learning in repeated pricing games, not an artifact of a specific parameterization.
