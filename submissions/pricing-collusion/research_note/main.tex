...
We present an agent-executable simulation framework that models repeated Bertrand competition under logit demand, trains five classes of pricing agents (Q-learner, SARSA, Policy Gradient, Tit-for-Tat, Competitive), and evaluates a panel of four detection auditors (Price-Cost Margin, Deviation-Punishment, Counterfactual Simulator, Welfare Analyst) across 540 parameterized simulations spanning three market presets, three memory lengths, six agent matchups, and shock/no-shock conditions.
We find that \textbf{TODO: fill after experiments} and that auditor agreement reaches \textbf{TODO} across majority-vote aggregation.
...
Exploration decays linearly from $\varepsilon = 1.0$ to $\varepsilon = 0.01$ over the first 40\% of $T = 500{,}000$ training rounds.
...
The 540 simulations span all combinations; each (agent matchup $\times$ memory $\times$ preset $\times$ shock) cell is replicated with 5 random seeds, and we report mean $\pm$ standard deviation for all metrics.
Statistical significance of supra-competitive pricing is assessed with Welch's $t$-test against the Nash price ($p < 0.05$); effect sizes are reported as Cohen's $d$.

\begin{table}[h]
\centering
\small
\caption{Experimental factor matrix. $6 \times 3 \times 3 \times 2 \times 5 = 540$ total simulations.}
\label{tab:factors}
\begin{tabular}{llll}
\toprule
...
Seeds & 5 & 0, 1, 2, 3, 4 & Controls init, $\varepsilon$-decay randomness \\
\midrule
\textbf{Total} & & \textbf{540 simulations} & \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Mean collusion index $\Delta$ (e-commerce preset, no shock, mean over 5 seeds). \textbf{TODO: fill after experiments.}}
\label{tab:heatmap}
\begin{tabular}{lccc}
\toprule
\textbf{Matchup} & \textbf{$M=1$} & \textbf{$M=3$} & \textbf{$M=5$} \\
\midrule
Q vs.\ Q       & \textbf{TODO} & \textbf{TODO} & \textbf{TODO} \\
SARSA vs.\ SARSA & \textbf{TODO} & \textbf{TODO} & \textbf{TODO} \\
PG vs.\ PG     & \textbf{TODO} & \textbf{TODO} & \textbf{TODO} \\
Q vs.\ SARSA   & \textbf{TODO} & \textbf{TODO} & \textbf{TODO} \\
Q vs.\ TFT     & \textbf{TODO} & \textbf{TODO} & \textbf{TODO} \\
Q vs.\ Comp    & \textbf{TODO} & \textbf{TODO} & \textbf{TODO} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Pairwise auditor agreement and panel method performance. \textbf{TODO: fill after experiments.}}
\label{tab:agreement}
\begin{tabular}{lcc}
\toprule
\textbf{Auditor pair / Method} & \textbf{Agreement rate} & \textbf{F1 score} \\
\midrule
PCM vs.\ DP         & \textbf{TODO} & --- \\
PCM vs.\ CF         & \textbf{TODO} & --- \\
PCM vs.\ WA         & \textbf{TODO} & --- \\
DP vs.\ CF          & \textbf{TODO} & --- \\
DP vs.\ WA          & \textbf{TODO} & --- \\
CF vs.\ WA          & \textbf{TODO} & --- \\
\midrule
...
\textbf{TODO: report slope and $p$-value after experiments.}
...
\textbf{TODO: fill recovery times and detection rate changes after experiments.}
...
Any AI coding agent can reproduce the full 540-simulation experiment by following the skill steps: set up the virtual environment, run unit tests, execute \texttt{run.py}, and validate with \texttt{validate.py}.
