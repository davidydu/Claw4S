...
The full sweep comprises 16 training runs (8 widths $\times$ 2 label types), completing in under 3 minutes on CPU.

\subsection{Interpolation Threshold}

We define the interpolation threshold as the smallest parameter count achieving $\geq 99\%$ training accuracy. For random labels, this occurs at a substantially higher parameter count than for structured labels, reflecting the fact that structured data contains exploitable patterns that reduce the effective number of parameters needed.

\subsection{Transition Sharpness}

The sigmoid fit to training accuracy vs.\ log-parameters yields the sharpness coefficient $k$. For random labels, we observe the transition concentrated over approximately one order of magnitude in parameter count.

\subsection{Memorization vs.\ Generalization}

As predicted, test accuracy on random labels remains near chance level ($\sim 10\%$) regardless of model size. This confirms that memorization capacity is orthogonal to generalization: a network can perfectly memorize random labels without learning any transferable features.

For structured labels, test accuracy increases with model size, demonstrating that when labels carry genuine structure, larger models can both memorize training data and extract generalizable patterns.

\section{Discussion}

Our findings replicate the core result of Zhang et al.\ (2017) in a controlled synthetic setting: neural networks can memorize random labels, but this requires more capacity than fitting structured data. The sigmoid characterization of the transition adds a quantitative measure (sharpness $k$) that was not reported in the original work.

\textbf{Limitations.} We study only two-layer MLPs on Gaussian data. Real-world datasets and deeper architectures may exhibit different thresholds and sharpness profiles. We use a single random seed; multiple seeds would better quantify variance. The Adam optimizer may behave differently from SGD in terms of the convergence trajectory near the threshold.

\section{Conclusion}

We provide a reproducible, quantitative measurement of the interpolation threshold in neural networks, confirming that the transition from partial to full memorization follows a sigmoid curve in log-parameter space. The experiment runs entirely on CPU in under 3 minutes and is designed for full reproducibility by AI agents.
