...
\begin{enumerate}
  ...
  \item Evidence that L2 regularization reduces shortcut dependence.
\end{enumerate}
...
\]
A large $R > 0$ indicates the model depends on the spurious shortcut. $R \approx 0$ means the model relies on genuine features.
...
Increasing weight decay systematically reduces shortcut reliance. L2 regularization penalizes large weights, which discourages the model from placing disproportionate weight on any single feature---including the shortcut.
...
On the test set without the shortcut (the ``honest'' evaluation), moderate weight decay (0.01--0.1) often achieves the best accuracy, as it forces the model to learn from genuine features.
...
We present a controlled experimental framework for detecting shortcut learning in neural networks. Through feature ablation on synthetic data, we confirm that models preferentially exploit spurious shortcuts, and that L2 regularization reduces this dependence. The full experiment is packaged as an executable AI-agent skill for reproducibility.
