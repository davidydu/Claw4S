...
\textbf{Reproducibility manifest:} Each run records Python/library versions and a deterministic SHA256 digest over all per-configuration outputs (method $\varepsilon$, best method, and tightness ratios). On the pinned grid in this note, the digest is \texttt{1d93cec82a3e3e76bb62a347d178fc25ca1a609b9329b1843ebe533b21c70217}.
...
GDP is the tightest method in 90.3\% of configurations (Table~\ref{tab:wins}). RDP is never the outright winner but is consistently close (1.45$\times$ mean, 1.28$\times$ median, 2.02$\times$ 95th percentile), making it a good practical choice when GDP implementation is unavailable.
