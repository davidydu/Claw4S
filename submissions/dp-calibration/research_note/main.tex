...
A key finding is that advanced composition \emph{never} improves over naive in our grid when $\sigma \leq 10$. The theorem requires $\varepsilon_{\text{step}} \ll 1$ for the $\sqrt{T}$ improvement to manifest; with $\sigma = 1.0$, the per-step $\varepsilon \approx 4.84$, and the $T \cdot \varepsilon(e^{\varepsilon} - 1)$ term dominates. Only at $\sigma \geq 10$ (where $\varepsilon_{\text{step}} \approx 0.48$) does advanced composition provide meaningful improvement over naive.
...
\textbf{Practical implications.} For practitioners choosing an accounting method: (1) GDP should be the default for Gaussian mechanisms, especially at large $T$; (2) RDP remains competitive and is easier to extend to non-Gaussian mechanisms; (3) advanced composition is only useful for very high noise ($\sigma > 10$) regimes; (4) the choice of accounting method can affect the required noise by 2--10$\times$.
