...
Parameter counts range from ${\sim}7{,}200$ ($h = 32$) to ${\sim}19{,}600$ ($h = 128$), all well under 100K.

\subsection{Training}

We use the AdamW optimizer \citep{loshchilov2019adamw} with learning rate $10^{-3}$, $\beta = (0.9, 0.98)$, and cross-entropy loss. Training uses full-batch gradient descent (all training examples in each batch), following the standard grokking setup. Each run trains for up to 5{,}000 epochs with early stopping when both train and test accuracy exceed 99\% for three consecutive evaluation points. Metrics are logged every 100 epochs.
...
\begin{itemize}
  ...
  \item \textbf{Grokking}: Both accuracies reach $95\%$, but test accuracy lags train by $> 500$ epochs. Delayed generalization.
  \item \textbf{Comprehension}: Both accuracies reach $95\%$ with test lagging by $\leq 500$ epochs. Fast generalization.
\end{itemize}
...
\begin{itemize}
  ...
  \item Hidden dimension $h \in \{32, 64, 128\}$ (3 values)
\end{itemize}
...
Wider models ($h = 128$) tend to grok or comprehend more readily than narrow models ($h = 32$) at the same weight decay and dataset fraction. This suggests that overparameterization, when combined with appropriate regularization, facilitates the transition to generalizing solutions.
...
\paragraph{Limitations.} Our study uses a single arithmetic operation (addition mod 97), a single optimizer (AdamW), and a single seed. The grokking gap threshold (500 epochs) is somewhat arbitrary. Extending to multiplication, varying seeds for confidence intervals, and exploring additional optimizers would strengthen these findings.
