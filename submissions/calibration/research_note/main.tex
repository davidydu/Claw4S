...
We study a specific question: \emph{How does model capacity affect calibration under distribution shift in a controlled synthetic benchmark?}
Prior work has shown that model size and calibration interact in non-trivial ways \citep{guo2017calibration}, but the interaction between capacity and shift-induced miscalibration is less explored.
...
\begin{enumerate}
    ...
    \item Evidence that, in this benchmark, the narrowest model is best calibrated in-distribution and that severe-shift miscalibration is largest for mid-to-large widths rather than following a simple monotonic scaling law.
...
Reliability diagrams (Figure~\ref{fig:reliability}) for the width-256 model show increasing departure from the diagonal---particularly in high-confidence bins---as shift grows.
