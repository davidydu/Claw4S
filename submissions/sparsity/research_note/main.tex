...
correlates with generalization gap (Spearman $\rho = -0.857$, $p = 0.007$)
...
sparsity dynamics depending on the task structure. These results suggest
...
The strongest finding is a strong negative correlation between
zero activation fraction and generalization gap ($\rho = -0.857$,
$p = 0.007$). Experiments with higher zero fractions (more sparse
activations) tend to have smaller generalization gaps and higher test
accuracy. The change in zero fraction during training shows a positive
but not conventionally significant trend with test accuracy
($\rho = 0.667$, $p = 0.071$): experiments that \emph{increase} their
...
test accuracy ($\sim$0.58 and $\sim$0.73 respectively).
...
with generalization across ReLU MLP experiments ($\rho = -0.857$ vs.\
