...
Our results confirm that GPT-4o's expanded 200K vocabulary achieves the best equity (avg.\ tax 1.75$\times$), and that no single tokenizer dominates across all scripts.
...
German    & 1.19$\times$ & \textbf{1.04$\times$} & 1.26$\times$ & 1.18$\times$ \\
French    & 1.22$\times$ & \textbf{1.07$\times$} & 1.31$\times$ & 1.21$\times$ \\
Spanish   & 1.24$\times$ & \textbf{1.09$\times$} & 1.33$\times$ & 1.24$\times$ \\
Russian   & 2.07$\times$ & \textbf{1.24$\times$} & 1.73$\times$ & 1.47$\times$ \\
Chinese   & 4.94$\times$ & 3.38$\times$ & 4.15$\times$ & \textbf{2.62$\times$} \\
Japanese  & 4.48$\times$ & 3.41$\times$ & 3.84$\times$ & \textbf{2.59$\times$} \\
Korean    & 4.03$\times$ & \textbf{2.67$\times$} & 3.99$\times$ & 2.79$\times$ \\
Hindi     & 4.20$\times$ & \textbf{1.47$\times$} & 3.69$\times$ & 3.89$\times$ \\
Arabic    & 3.02$\times$ & \textbf{1.53$\times$} & 3.22$\times$ & 1.69$\times$ \\
Turkish   & 1.67$\times$ & \textbf{1.37$\times$} & 1.92$\times$ & 1.49$\times$ \\
Vietnamese& 2.19$\times$ & 1.36$\times$ & 2.48$\times$ & \textbf{1.27$\times$} \\
Finnish   & 1.64$\times$ & \textbf{1.38$\times$} & 1.68$\times$ & 1.62$\times$ \\
Hebrew    & 3.94$\times$ & 1.69$\times$ & 3.56$\times$ & \textbf{1.61$\times$} \\
\midrule
\textbf{Avg.\ tax} & 2.76$\times$ & \textbf{1.75$\times$} & 2.63$\times$ & 1.90$\times$ \\
\textbf{Max tax}   & 4.94$\times$ & \textbf{3.41$\times$} & 4.15$\times$ & 3.89$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{GPT-4o}: avg.\ 1.75$\times$, max 3.41$\times$ (Japanese)
  \item \textbf{Qwen2.5}: avg.\ 1.90$\times$, max 3.89$\times$ (Hindi)
  \item \textbf{Mistral}: avg.\ 2.63$\times$, max 4.15$\times$ (Chinese)
  \item \textbf{GPT-4}: avg.\ 2.76$\times$, max 4.94$\times$ (Chinese)
\end{enumerate}
...
Chinese tax drops from 4.94$\times$ (GPT-4) to 3.38$\times$ (32\% improvement); Hindi drops from 4.20$\times$ to 1.47$\times$ (65\% improvement); Arabic drops from 3.02$\times$ to 1.53$\times$ (49\% improvement).
...
Despite GPT-4o's overall lead, Qwen2.5 outperforms it on CJK languages (Chinese: 2.62$\times$ vs.\ 3.38$\times$) and Vietnamese (1.27$\times$ vs.\ 1.36$\times$), reflecting its training data composition.
However, Qwen2.5 performs poorly on Hindi (3.89$\times$ vs.\ GPT-4o's 1.47$\times$), illustrating that tokenizer equity is inherently a multi-objective problem.

\paragraph{The CJK floor.}
Even the best-performing tokenizer for Chinese (Qwen2.5 at 2.62$\times$) still imposes substantial tax.
...
GPT-4o's 200K vocabulary achieves the best overall equity (avg.\ tax 1.75$\times$), while Qwen2.5 leads on CJK.
